...
export default function ({ children, device }) {
    ...
    const name = String(
        device?.name || String(device?.variant).charAt(0).toUpperCase() + String(device?.variant).substring(1)
    );
        ...
        >
                        ...
                        <Portal>
                            <Menu.Positioner>
                                <Menu.Content display={"flex"} flexDir={"column"} gap={"0.25rem"}>
                                    <Menu.ItemGroup>
                                        <Menu.Item value="control_device" transition={".2s ease all"}>
                                            Gerät steuern
                                        </Menu.Item>
                                        <Menu.Item value="show_device_data" transition={".2s ease all"}>
                                            Daten anzeigen
                                        </Menu.Item>
                                    </Menu.ItemGroup>
            ...
            {!isSm && (
                <Box display={"flex"} gap={"0.25rem"}>
                    <Text userSelect={"none"} flex={1} textAlign={"right"} fontSize={"2xs"} color={"fg.muted"}>
                        #
                    </Text>
                    <Text userSelect={"text"} fontSize={"2xs"} color={"fg.muted"}>
                        {device?.uid}
                    </Text>
                </Box>
            )}
