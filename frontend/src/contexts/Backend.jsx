...
export function BackendProvider({ children }) {
    ...
    useEffect(() => {
        ...
        s.on("device:update", ({ uid, device }) => {
            setDevices((prev) => prev.map((d) => (d.uid === uid ? { ...d, ...device, uid } : d)));
        });

        s.on("room:update", ({ uid, room }) => {
            setRooms((prev) =>
                prev
                    .map((r) => (r.uid === uid ? { ...r, ...room, uid } : r))
                    .sort((roomA, roomB) => roomA?.index || 0 - roomB?.index)
            );
        });

        s.on("room:add", ({ room }) => {
            setRooms((prev) => [...prev, room].sort((roomA, roomB) => roomA?.index || 0 - roomB?.index));
        });

        s.on("room:remove", ({ uid }) => {
            setRooms((prev) =>
                prev.filter((r) => r.uid != uid).sort((roomA, roomB) => roomA?.index || 0 - roomB?.index)
            );
        });
    ...
    return (
        <>
            <Context.Provider value={{ socket, devices, rooms }}>{children}</Context.Provider>
        </>
    );
}
...
export function useBackend() {
    ...
    if (!context) {
        throw new Error("context for useBackend missing!");
    }
